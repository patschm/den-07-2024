...
internal class Program
{
    ...
    private static void LoggingWithSettings()
    {
    ...
    private static void LoggingCustom()
    {
        var builder = new ConfigurationBuilder();
        builder.SetBasePath(Environment.CurrentDirectory);
        builder.AddJsonFile("appsettings.json");
    ...
    private static void DependencyInjection()
    {
        ...
        // .NET 8
        builder.AddKeyedTransient<ICounter, Counter>("good");
        builder.AddKeyedTransient<ICounter, CrappyCounter>("bad");
        ...
        var good = provider.GetRequiredKeyedService<ICounter>("good");
        var bad = provider.GetRequiredKeyedService<ICounter>("bad");

#pragma warning disable
        // Querying injector:
        void AskingInjector([FromKeyedServices("good")]ICounter counter)
        {
            //...
        }
        // Or
        void FromInjector(IKeyedServiceProvider provider)
        {
            var obj = provider.GetRequiredKeyedService<ICounter>("good");
        }
    }
    private static void AllInOneOld()
    {
        ...
        // Will be replaced by CreateApplicationBuilder.
    ...
    private static void AllInOneApp()
    {
